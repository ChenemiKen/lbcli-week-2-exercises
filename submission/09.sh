...
UTXO_VALUE=$(bitcoin-cli -regtest decoderawtransaction $BASE_TX | jq -r  --argjson idx "$UTXO_VOUT_INDEX" '.vout[$idx].value' | awk '{printf "%.0f\n", $1 * 100000000}')
...
CHANGE_AMOUNT=$(($PAYMENT_AMOUNT - $FEE_SATS))
check_cmd "Change calculation" "CHANGE_AMOUNT" "$CHANGE_AMOUNT"

# Convert amounts to BTC for createrawtransaction
#  awk '{printf "%.0f\n", $1 * 100000000}'
PAYMENT_BTC=$(awk -v amount="$PAYMENT_AMOUNT"  'BEGIN {printf "%.8f", amount / 100000000}')
CHANGE_BTC=$(awk -v amount="$CHANGE_AMOUNT" 'BEGIN {printf "%.8f", amount / 100000000}')
...
CHANGE_OUTPUT_AMOUNT=$(bitcoin-cli -regtest decoderawtransaction $RAW_TX | jq -r '.vout[1].value *100000000')
CHILD_SEND_AMOUNT=$(($CHANGE_OUTPUT_AMOUNT - $CHILD_FEE_SATS))
...
# WRITE YOUR SOLUTION BELOW:
TIMELOCK_INPUTS='[{"txid":"'$SECONDARY_TXID'","vout":0}]'
...
SECONDARY_OUTPUT_VALUE=$(bitcoin-cli -regtest decoderawtransaction $SECONDARY_TX | jq -r '.vout[0].value *100000000')
